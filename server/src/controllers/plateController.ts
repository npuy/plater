...
export const createPlate = (req: Request, res: Response): void => {
  try {
    ...
    const { plate_number, date, time, description } =
      req.body as CreatePlateDTO;

    if (!plate_number || !date) {
      res
        .status(400)
        .json({ error: "Faltan campos requeridos: plate_number y date" });
      return;
    }

    // Validar formato de hora si se proporciona (HH:MM)
    ...
    const stmt = db.prepare(`
      INSERT INTO plates (plate_number, image_path, date, time, description)
      VALUES (?, ?, ?, ?, ?)
    `);
    ...
    const newPlate = db
      ...
      .get(result.lastInsertRowid) as Plate;
